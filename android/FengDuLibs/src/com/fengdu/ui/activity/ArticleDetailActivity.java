...
import com.fengdu.widgets.MyWebView;
...
//            mArticlePostmetaTv.setText(Html.fromHtml(mArticlePostmetaStr));
	...
	private String parseArticleContent(boolean isShare,Document content){
		//　去掉广告
		if(content.getElementById("arc_468")!=null){
			content.getElementById("arc_468").remove();
		}
		// 批量处理img标签 链接地址、宽高设置
        ...
        for (Element element : pngs) {
            ...
            if(!isShare){
					element.attr("style","max-width: 80%; height: auto;");
			}else{
        		...
        		element.attr("style", "width:85%;");
            }
        }
//        tags = content.getElementById("tags").toString();
        Element article = content.getElementById("artText");
...
//	}
	...
	private String makeHtml(ArticleItemBean bean){
		...
		try {
			...
			document.getElementById("title").append(mArticleBean.getTitle());
			document.getElementById("tagss").append(tags);
			Document content = Jsoup.parse(doc.toString());
			document.getElementById("span").append(mArticlePostmetaStr);
			document.getElementById("article").append(parseArticleContent(true,content));
			File file = new File(CommonConfig.getUploadHtmlPath());
			if(!file.exists()){
				file.mkdirs();
			}
			File files = new File(CommonConfig.getUploadHtmlPath()+bean.getMd5()+".html");
			if(files.exists()){
				return CommonConfig.getUploadHtmlPath()+bean.getMd5()+".html";
			}else{
				fw = new FileWriter(CommonConfig.getUploadHtmlPath()+bean.getMd5()+".html");
...
//				if(mReturnBean!=null && mReturnBean.getCode()==200){
